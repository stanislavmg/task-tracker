...
class DeleteUserFromTicket final : public userver::server::handlers::HttpHandlerBase {
public:
    static constexpr std::string_view kName = "handler-v1-delete-watcher-from-ticket";

    DeleteUserFromTicket(const userver::components::ComponentConfig& config,
    ...
    std::string HandleRequestThrow(
        ...
        auto result = DeleteUserIdFromTicket(pg_cluster_, *user_id, ticket_id);
...
void AppendDeleteUserFromTicket(userver::components::ComponentList& component_list) {
  component_list.Append<DeleteUserFromTicket>();
}
